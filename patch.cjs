...
const regex = /const publishSpeakerResult = async \(diagResult: any, isCalibration = false, phase: 'PROBE' \| 'FINAL' = 'FINAL'\) => \{\s+if \(!diagResult\) return;\s+if \(diagResult\.identitySource === 'VERIFIED'\) \{[\s\S]*?\} else if \(phase === 'FINAL'\) \{[\s\S]*?\}\s+if \(clientWs\.readyState === clientWs\.OPEN\) \{\s+clientWs\.send\(JSON\.stringify\(\{/m;
